...
fn print_segment_args(segment: &PathSegment, cache: Option<&Cache>) -> String {
    match &segment.args {
        GenericArgs::AngleBracketed { args, constraints: bindings } => {
            ...
            for arg in args.iter() {
                let sym = match arg {
                    ...
                    GenericArg::Const(constant) => "<const>".to_string(),
            ...
            for binding in bindings.iter() {
                ...
                let value = match binding.kind {
                    ...
                    clean::AssocItemConstraintKind::Bound { ref bounds } => format!(":{:?}", bounds),
                };
    ...
    match type_ {
        ...
        clean::Type::QPath(qpathdata) => {
            format!(
                ...
                if qpathdata.trait_.is_some() {
                    print_path(qpathdata.trait_.as_ref().unwrap(), cache)
                } else {
                    "_".to_string()
                },
...
pub(crate) fn _same_type_hard_mode(
    ...
    let callable = match output_type {
        ...
        clean::Type::Pat(_, _) => CallType::_NotCompatible,
    };
    // println!("{:?}", callable);``
    callable
}
...
fn new_segments_without_lifetime(
    ...
    for old_path_segment in old_path_segments {
        ...
        if let clean::GenericArgs::AngleBracketed { args, constraints: bindings } = generic_args {
...
pub(crate) fn is_binding_match(a: &AssocItemConstraint, b: &AssocItemConstraint) -> bool {
    if a.assoc.name.as_str() != b.assoc.name.as_str() || a.kind != b.kind {
        false
    } else {
        true
    }
...
pub(crate) fn is_type_match(ty_a: &clean::Type, ty_b: &clean::Type) -> bool {
    match (ty_a, ty_b) {
        ...
        (clean::Type::Path { path: a }, clean::Type::Path { path: b }) => {
            ...
            if let (
                GenericArgs::AngleBracketed { args: a_args, constraints: a_bindings },
                GenericArgs::AngleBracketed { args: b_args, constraints: b_bindings },
            ) = (a_args, b_args)
            {
    ...
    1 + match type_ {
        ...
        clean::Type::Path { path } => {
            ...
            for path_segment in segments {
                ...
                match generic_args {
                    clean::GenericArgs::AngleBracketed { args, constraints: bindings } => {
                        for generic_arg in args.iter() {
